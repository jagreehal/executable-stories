...
| Incomplete collection | Emitted by Vitest, Jest and Playwright | Emitted by JUnit 5; not by Go, Ruby, Rust, pytest or xUnit | The JUnit Platform reports containers as well as tests, and anything that failed or was skipped never reached the tests inside it. That matters most where the platform is generous: an enclosing class comes back successful even when a `@TestFactory` in it failed, so without the incomplete list a broken factory is indistinguishable from a class whose scenarios were deleted |
| Executed-file inventory | Emitted by Vitest, Jest and Playwright | Emitted by JUnit 5 and xUnit; not by Go, Ruby, Rust or pytest | Each host enumerates executed files differently. The JUnit Platform listener and xUnit's recording attribute both already run for every test, so the classes they saw are the inventory. Where it is omitted the consequence is bounded and visible: a file whose last scenario was deleted keeps that scenario until the file is removed from the working tree |
