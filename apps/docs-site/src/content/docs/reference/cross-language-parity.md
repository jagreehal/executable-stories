...
| **Run scope** | A run reports `runScope` — `"full"`, `"filtered"`, or absent — so consumers know whether it may retire a scenario it no longer reports. Only `"full"` retires; absent keeps and warns | Vitest/Jest (`testNamePattern`), Playwright (`grep`/`grepInvert`); Cypress declares it | Go (`-run`), Ruby (Minitest plugin options), pytest (`-k`, `-m`, `--deselect`, `--last-failed`, a node id, or a run that ended early), Rust (a positional filter, `--skip`, `--ignored`) detect it; JUnit 5 and xUnit declare it via `EXECUTABLE_STORIES_FILTERED` |
...
| Assembling a run | One reporter sees the whole run | Rust and Go write one run file per test binary | Cargo compiles each file under `tests/` into its own binary, and `go test ./...` builds one per package, so no single process sees the whole run and each writes the same default path. Folding them together in the adapter would misdate the results it carried: a run stamps everything in it with one commit sha and one pair of timestamps, and there is no reliable way from inside a test binary to tell which of them belong to the same invocation — `cargo test` runs doctests through `rustdoc`, under a different parent again. Until a run file can carry per-result provenance, or the formatter can take a set of runs, keep story tests in one target per project or give each its own output path |
