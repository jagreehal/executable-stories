...
| Executed-file inventory | Emitted by Vitest, Jest and Playwright | Emitted by xUnit; not by Go, Ruby, Rust, pytest or JUnit 5 | Each host enumerates executed files differently. xUnit's recording attribute already runs after every test in the assembly, so the classes it saw are the inventory. Where it is omitted the consequence is bounded and visible: a file whose last scenario was deleted keeps that scenario until the file is removed from the working tree |
