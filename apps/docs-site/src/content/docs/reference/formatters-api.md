...
The scenario → Markdown serializer, **`scenarioToMarkdown`**, lives in the internal `executable-stories-core` package (not in formatters). It is the single implementation behind both the HTML report's per-scenario "Copy as Markdown" button (`variant: "compact"`, a paste-sized excerpt) and standalone `<slug>.md` documents (the default full variant), so the two surfaces cannot drift. See [Core types & constants](/reference/core-api/).
...
With `JEV_API_KEY` set, `triage`, `goal`, and `review` ask Jev (TypeSafe AI) the questions their rules leave open: a suggested `covers` path and failure kind for unrouted failures, ratchet advisories for rewritten scenarios that check less, and an inferred change-type for untagged claims. Each answer carries its probability and none changes an exit code. See [Agent loops](/guides/agent-loops/#jev-judgments-where-the-rules-stop).
...
`--include`, `--exclude`, `--include-tags` and `--exclude-tags` name any selector that
matched no test case, so a filter whose path moved says so instead of quietly
filtering nothing.
