...
- **Jest** — In `reporters`: `["jest-executable-stories/reporter", { title: "...", ... }]`.
- **Vitest** — In `new StoryReporter({ title: "...", ... })` in `vitest.config.ts`; import from `vitest-executable-stories/reporter`.
- **Playwright** — In the reporter entry: `["playwright-executable-stories/reporter", { title: "...", ... }]`.
