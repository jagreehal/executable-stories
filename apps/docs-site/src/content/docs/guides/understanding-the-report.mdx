...
- **Repeats show the change.** Consecutive snapshots with the same label are
  ...
  `total: 0 → 45`, so the reader sees what the step did rather than two blobs to
...
- **Labels are lanes.** Snapshot two entities (`Basket` and `Order`) and each
  ...
  a screenshot and a state, putting the screen next to the backend record it proves.
