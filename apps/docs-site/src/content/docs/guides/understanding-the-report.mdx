...
- **The `/states` catalog** (`state:<name>` tags) gives non-UI scenarios
  ...
  at two granularities: tags name states, `story.state()` shows them.
...
See the [agent artifact contract](/guides/agent-artifact-contract/#webmcp-in-the-readers-browser) for the differences that a browser forces.
