...
| Band | Meaning |
| --- | --- |
| 🔴 Uncovered | no claim or test correlates to this file, so review it first |
...
Annotations do not use line numbers, because line numbers detach the moment the patch is regenerated, which for AI-authored changes is constantly. Each annotation is anchored to the **content** of its changed lines plus surrounding context, and relocates in a regenerated patch the way `patch(1)` does, with bounded fuzz.

| State | Meaning |
| --- | --- |
| Anchored | exactly one confident match, so the annotation renders on its hunk |
| Ambiguous | the lines appear in more than one place, rendered visibly unattached, never guessed |
| Orphaned | the lines were rewritten, so it renders with a "could not locate in current patch" notice |
