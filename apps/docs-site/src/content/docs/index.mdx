...
description: Framework-native BDD-style tests with generated reports for Vitest, Jest, Playwright, and Cypress
template: splash
hero:
  tagline: Framework-native BDD-style tests with generated reports for Vitest, Jest, Playwright, and Cypress
  actions:
...
One source of truth. Code that executes. Reports that do not lie.

## What it does

We **add to** your existing test setup instead of replacing it. You keep your framework’s `describe` / `it` (or `test.describe` / `test`). Inside a test, call **`story.init()`** (or `story.init(task)` / `story.init(testInfo)` for Vitest/Playwright), then use **`story.given`**, **`story.when`**, **`story.then`** to document the scenario. Reporters and the formatter CLI turn that into Markdown, HTML, JUnit XML, Cucumber JSON, Cucumber Messages, and Cucumber HTML.
...
- **Reporter** — Generates reports from test metadata; if a test is skipped, failed, or todo, the output reflects that.
...
Same idea: **`story.init()`** at the start of the test (no task; Jest gets the test name from `expect.getState()`). Jest also exports top-level **`given`**, **`when`**, **`then`**, **`and`**, and **`but`** helpers if you prefer them:
...
**`story.init(testInfo)`** at the start of the test (pass **`testInfo`** from the callback), then **`story.given`** / **`story.when`** / **`story.then`**. If you want step callbacks to receive fixtures, call **`story.init({ page }, testInfo)`** instead:
...
This repo also ships adapters and example apps for Go, pytest, JUnit 5, Rust, and xUnit. The setup guides on this site focus on the JavaScript packages; see [Other adapters](/reference/other-adapters/) for the wider repo surface area.
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Status icons, headings, and what appears in generated reports"
    href="guides/understanding-the-report/"
  />
  <LinkCard
    title="Common issues"
    description="Troubleshooting missing reports, step errors, and framework gotchas"
  ...
  <LinkCard
    ...
    description="Lint rules per framework for init order, story context, and doc usage"
    href="reference/eslint-plugins/"
  />
  <LinkCard
    title="Formatters API"
    description="Programmatic report generation, CLI formats, notifications, and history"
    href="reference/formatters-api/"
  />
  <LinkCard
    title="Other adapters"
    description="Go, pytest, JUnit 5, Rust, and xUnit packages in this repo"
    href="reference/other-adapters/"
  />
  <LinkCard
