...
// ============================================================================
// Story Options with Callback Parameter (Vitest-specific)
// ============================================================================

story(
  "Story options with callback parameter",
  { tags: ["callback-pattern"], ticket: "VIT-001" },
  (stepsApi: StepsApi) => {
    doc.note("Options work with callback parameter pattern");
    const { given: gv, when: wh, then: th } = stepsApi;

    gv("callback parameter is used", () => {});
    wh("options are provided", () => {});
    th("both work together", () => {
      expect(true).toBe(true);
    });
  }
);
