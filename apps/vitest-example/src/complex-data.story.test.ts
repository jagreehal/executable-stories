 ...
 * it() + story.init(task) + story.note/json/table/code/mermaid/section (options objects).
 */
...
it('Deeply nested JSON structures', ({ task }) => {
  story.init(task);
...
it('Arrays of complex objects', ({ task }) => {
  story.init(task);
...
it('YAML configuration examples', ({ task }) => {
  story.init(task);
...
it('Various Mermaid diagram types', ({ task }) => {
  story.init(task);
...
it('Story with complex metadata structure', ({ task }) => {
  story.init(task, {
...
it('All complex data types in one story', ({ task }) => {
  story.init(task);
  ...
  story.code({
    label: 'Quick Start',
    content: `import { story } from 'executable-stories-vitest';
it('My Test', ({ task }) => {
  story.init(task);
