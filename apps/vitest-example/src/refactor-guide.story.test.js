...
describe('Part 3: Framework-native with story.init(task)', () => {
    it('Step 2 — Keep it(), add story.init(task): existing test appears in docs', ({ task, }) => {
        /**
...
describe('Part 4: Full patterns', () => {
    ...
    it('Step 3b — Framework-native test with story.init(task) in the same describe', ({ task, }) => {
        story.init(task);
        expect(multiply(7, 6)).toBe(42);
    });
    ...
    it('Calculator adds with a note', ({ task }) => {
        story.init(task);
        story.note('Using small numbers; the note appears in the generated Markdown.');
