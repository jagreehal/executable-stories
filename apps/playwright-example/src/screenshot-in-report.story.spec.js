...
test('Screenshot appears in generated report', async ({ page }, testInfo) => {
    ...
    await page.setContent('<!DOCTYPE html><html><body><h1>Hello</h1><p>This page is screenshot and linked in the story report.</p></body></html>');
...
test('Video is recorded and linked in report', async ({ page }, testInfo) => {
    story.init(testInfo);
    story.given('user is on a page');
    await page.setContent('<!DOCTYPE html><html><body><h1>Video example</h1></body></html>');
    story.when('the step runs');
    story.then('video path is in the story report');
    const testInfoWithVideo = testInfo;
    const video = typeof testInfoWithVideo.video === 'function'
    ...
    story.note('Videos are under test-results/; run `pnpm test:ui` or open playwright-report/ to watch.');
});
