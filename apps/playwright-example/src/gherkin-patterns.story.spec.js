...
test('API endpoint documentation', async ({}, testInfo) => {
    ...
    story.section({
        ...
        markdown: 'This endpoint handles user authentication and returns a JWT token.',
    });
    story.when('a POST request is made to /api/login');
    story.code({
        label: 'Request Headers',
        content: JSON.stringify({ 'Content-Type': 'application/json', Accept: 'application/json' }, null, 2),
        lang: 'json',
    });
...
for (const profile of userProfiles) {
    test(`${profile.name} features`, async ({}, testInfo) => {
        ...
        story.then(`the user should have access to ${profile.features.length} features`);
