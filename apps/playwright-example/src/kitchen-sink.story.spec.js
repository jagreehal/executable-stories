...
test('Kitchen sink – every story API method', async ({}, testInfo) => {
    story.init(testInfo, {
        tags: ['kitchen-sink'],
        ticket: 'KS-001',
        meta: { area: 'docs', priority: 'high' },
    });
    story.note('This test exercises every story.* method so generated docs include all of them.');
    story.tag('smoke');
    story.kv({ label: 'Framework', value: 'Playwright' });
    story.kv({ label: 'API Version', value: '1.0' });
    story.json({
        label: 'Config',
        value: { reporter: true, colocatedDocs: true },
    });
    story.code({
        label: 'Snippet',
        content: 'const x = 1;\nconst y = 2;',
    ...
    story.then('generated doc contains note, table, kv, json, code, link, section, mermaid, screenshot, custom');
