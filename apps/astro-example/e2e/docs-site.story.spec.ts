...
test("The explorer filters scenarios by search text", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText(/Gift wrapping is offered/i)).toBeVisible();
  // Non-matches are hidden (filtered client-side), not necessarily removed.
  await expect(page.getByText(/Checkout is blocked/i)).toBeHidden();
});
...
test("The explorer filters scenarios by status", async ({ page }, testInfo) => {
  ...
  await page.goto("/explorer/");
  await expect(page.getByText(/Checkout is blocked/i)).toBeVisible();
  await expect(page.getByText(/A returning customer/i)).toBeVisible();
  ...
  await expect(page.getByText(/Checkout is blocked/i)).toBeVisible();
  await expect(page.getByText(/A returning customer/i)).toBeHidden();
