 ...
 * - Timeouts and delays
 */
import { story, given, when, then, and, doc, arrange, act, assert } from "jest-executable-stories";
...
story("Async setup and teardown pattern", () => {
  ...
  arrange("database connection is established", async () => {
    ...
    // doc.kv("Connection ID", connection.id);
  });

  arrange("transaction is started", async () => {
    await delay(5);
    transactionId = `tx-${Date.now()}`;
    // doc.kv("Transaction ID", transactionId);
  });
...
story("Real-world async API test pattern", () => {
  ...
  act("GET request is made", async () => {
    ...
    // doc.json("GET Response", getResponse);
  });
  ...
  act("POST request is made", async () => {
    ...
    // doc.json("POST Response", postResponse);
  });
...
story("Async steps with runtime documentation", () => {
  ...
  given("async operation is prepared", async () => {
    await delay(5);
    // doc.note("Preparation complete");
    // doc.kv("Prep Time", "5ms");
  });

  when("async data is fetched", async () => {
    const _data = await fetchUser("789");
    // doc.json("Fetched Data", data);
    // doc.tag(["async", "fetched"]);
  });

  then("runtime docs contain async results", async () => {
    await delay(5);
    // doc.kv("Final Verification", "passed");
    expect(true).toBe(true);
  });
});
