...
describe("reportFromStoryEntries (loader adapter)", () => {
  it("builds a resolvable report from flat story entries", () => {
    const report = reportFromStoryEntries([
      { id: "a", title: "A", status: "passed", tags: ["smoke"], feature: { title: "Feat", sourceFile: "f.ts" }, run: { runId: "r1", finishedAtMs: 123 } },
      { id: "b", title: "B", status: "failed", feature: { title: "Feat", sourceFile: "f.ts" }, run: { runId: "r1", finishedAtMs: 123 } },
    ]);
    expect(report.runId).toBe("r1");
    expect(report.finishedAtMs).toBe(123);
    const verified = resolveVerification("a", report);
    expect(verified.status).toBe("verified");
    const failing = resolveVerification("smoke", report); // tag match -> only "a"
    expect(failing.status).toBe("verified");
    expect(resolveVerification("b", report).status).toBe("failing");
  });
});
