 ...
 */
import { story, given, when, steps as bddSteps, step, arrange, doc } from "../../index.js";
...
doc.story("Alt API story", (steps) => {
  ...
  bddSteps.then("result is verified", () => {
...
story("Alt API story without steps param", () => {
  ...
  step.then("standalone verification", () => {
    expect(true).toBe(true);
  });
});
...
story("Optional step callback story", () => {
  ...
  step.then("outcome with no impl", () => {
    expect(true).toBe(true);
  });
});
