...
function isTestCallExpression(node: CallExpression): boolean {
  ...
  if (callee.type === "CallExpression") {
    ...
    if (inner.object.name !== "test" && inner.object.name !== "it") return false;
    return inner.property.type === "Identifier" && inner.property.name === "each";
  }
  return false;
}
...
const rule: Rule.RuleModule = {
  meta: {
    ...
    docs: {
      description:
        "Require doc.story(title) to be called inside a test/it callback in Vitest.",
      recommended: true,
    },
    schema: [],
    messages: {
  ...
  create(context) {
    ...
    return {
      ...
      CallExpression(node: CallExpression) {
        ...
        // Skip when second argument is a function (callback pattern) or an identifier that refers to a function
        if (node.arguments.length >= 2) {
          const second = node.arguments[1];
          if (isFunctionNode(second)) return;
          if (second.type === "Identifier" && namedFunctions.has(second.name)) return;
        }
