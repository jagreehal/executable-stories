...
const config = [
  {
    ...
    rules: {
      "vitest-executable-stories/require-story-context-for-steps": "error" as const,
    },
  },
];
...
describe("require-story-context-for-steps", () => {
  it("allows steps inside story callback", () => {
    const code = `
      import { story, given, when, then } from "vitest-executable-stories";
      story("Login", () => {
        given("a user", () => {});
        when("they sign in", () => {});
  ...
  it("allows steps inside doc.story callback", () => {
    const code = `
      import { doc, given } from "vitest-executable-stories";
      doc.story("Login", () => {
        given("a user", () => {});
      });
    `;
  ...
  it("allows steps via steps/step object inside story callback", () => {
    const code = `
      import { story, steps, step } from "vitest-executable-stories";
      story("Login", () => {
        steps.given("a user", () => {});
        step.then("ok", () => {});
      });
  ...
  it("allows steps inside a named callback passed to story()", () => {
    const code = `
      import { story, given } from "vitest-executable-stories";
      const defineSteps = () => {
        given("a user", () => {});
      };
  ...
  it("reports step call outside story callback", () => {
    const code = `
      import { given } from "vitest-executable-stories";
      given("a user", () => {});
    `;
    const messages = linter.verify(code, config);
    expect(messages).toHaveLength(1);
    expect(messages[0].ruleId).toBe(
      "vitest-executable-stories/require-story-context-for-steps",
    );
  });
});
