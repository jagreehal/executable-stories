...
const allEntries: ReportDocEntry[] = [
  { kind: "note", phase: "static", text: "Seeded from the standard fixture cart." },
  { kind: "tag", phase: "static", names: ["regression-guard", "finance-approved"] },
  { kind: "kv", phase: "static", label: "Gateway response", value: { code: "declined", retriable: false } },
  {
    kind: "code",
    phase: "static",
    label: "Pricing call",
  ...
  { kind: "link", phase: "static", label: "Pricing policy", url: "https://example.com/policy" },
  {
    kind: "section",
    phase: "static",
...
export const AllKinds: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText("Seeded from the standard fixture cart.")).toBeVisible(); // note
    await expect(canvas.getByText("Discount tiers")).toBeVisible(); // table
    await expect(canvas.getByRole("link", { name: "Pricing policy" })).toBeVisible(); // link
    await expect(canvas.getByRole("img", { name: "Search result card" })).toBeVisible(); // screenshot
