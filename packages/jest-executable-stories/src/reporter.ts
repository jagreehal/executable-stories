/**
 * Jest reporter that reads story metadata emitted by jest-executable-stories tests
 * and writes Markdown user-story docs.
 ...
 * with user-story style sections. Uses onRunStart for start time and onRunComplete for report.
 */
...
/**
 * Jest reporter that generates Markdown documentation from scenario tests.
 *
 * Reads story metadata emitted by jest-executable-stories and generates user-story style Markdown.
 ...
 * // jest.config.ts
 * export default {
 *   reporters: ["default", ["jest-executable-stories", { output: "docs/user-stories.md" }]],
 * };
 * ```
 */
export default class StoryReporter {
  ...
  constructor(_globalConfig?: unknown, options?: StoryReporterOptions) {
    // Jest passes (globalConfig, options); direct usage may pass (options) only
    const opts: StoryReporterOptions = options ?? (_globalConfig as StoryReporterOptions) ?? {};
    this.options = {
      title: opts.title ?? "User Stories",
      description: opts.description ?? "",
      includeFrontMatter: opts.includeFrontMatter ?? false,
      output: opts.output,
      permalinkBaseUrl: opts.permalinkBaseUrl,
      enableGithubActionsSummary: opts.enableGithubActionsSummary ?? true,
      includeSummaryTable: opts.includeSummaryTable ?? false,
      includeMetadata: opts.includeMetadata ?? true,
      metadata: {
        date: opts.metadata?.date ?? "iso",
        packageVersion: opts.metadata?.packageVersion ?? true,
        gitSha: opts.metadata?.gitSha ?? true,
      },
      includeJson: opts.includeJson ?? false,
      json: {
        outputFile: opts.json?.outputFile,
        includeDocs: opts.json?.includeDocs ?? "all",
      },
      coverage: {
        include: opts.coverage?.include ?? false,
        file: opts.coverage?.file ?? "coverage/coverage-final.json",
      },
      groupBy: opts.groupBy ?? "file",
      scenarioHeadingLevel: opts.scenarioHeadingLevel,
      stepStyle: opts.stepStyle ?? "bullets",
  ...
  onRunComplete(_contexts: Set<unknown>, results: JestAggregatedResult): void {
    // Key by (file::scenario) to prevent collisions across files
    const scenarios = new Map<string, ScenarioWithMeta>();
    const root = process.cwd();
    const fileResults = new Map<string, JestTestFileResult>();
    ...
    for (const report of reports) {
      const sourcePath = path.relative(root, report.testFilePath);
      ...
      for (const meta of report.scenarios ?? []) {
        ...
        const matchingTests = (fileResult?.testResults ?? []).filter((test) => {
          const ancestors = Array.isArray(test.ancestorTitles) ? test.ancestorTitles : [];
          return ancestors.length > 0 && ancestors[ancestors.length - 1] === meta.scenario;
        });
  ...
  private readStoryReports(): StoryFileReport[] {
    ...
    const files = fg.sync("**/*.json", { cwd: outputDir, onlyFiles: true, absolute: true });
