...
function scenarioImpl(
  ...
  descFn(title, () => {
    ...
    if (existing) {
      ...
      storyRegistry.set(testPath, [story]);
      registerAfterAll();
    }
...
/**
 * Define a scenario (user story). Each step becomes one Jest test.
 * Scenario metadata is captured at registration time and written to disk
