...
import {
  ...
  failedScenario,
  passedScenario,
} from './html/storybook/fixtures';
import { RunDiffHtmlFormatter } from './run-diff-html';
...
export const Regressions: StoryObj = {
  render: () => {
    ...
    const html = formatter.format(
      buildDiff([regressedDiff(), regressedDiff()]),
    );
...
export const MixedChanges: StoryObj = {
  render: () => {
    const formatter = new RunDiffHtmlFormatter({ title: 'Run Comparison' });
    const html = formatter.format(
      buildDiff([regressedDiff(), fixedDiff(), addedDiff(), removedDiff()]),
    );
    return inIframe(html);
  },
};
