/**
 * Inline report JavaScript — browser-side string constants for the HTML report.
 *
 * Extracted from template.ts so that module stays focused on assembling the
 * document. These are large, self-contained JS source strings (theme toggle,
 * search / filter / collapse, markdown parsing, html-embed blob open) injected
 * verbatim into a <script> tag by generateScript()/generateEsmScript().
 */

/** Theme-related JavaScript (only included when darkMode is enabled) */
export const JS_THEME = `
// Theme management
