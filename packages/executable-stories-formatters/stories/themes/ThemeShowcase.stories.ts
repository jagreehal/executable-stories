...
import { renderDocCode, renderDocTable, renderDocNote } from "../../src/formatters/html/renderers/doc-entries";
...
const docDeps = { escapeHtml, syntaxHighlighting: false, markdownEnabled: false, mermaidEnabled: false };

function buildSampleHtml(): string {
  const summary = renderSummary({ total: 12, passed: 9, failed: 2, skipped: 1 }, {});
  ...
  const table = renderDocTable(
    { kind: "table", phase: "static", label: "Test Matrix", columns: ["Input A", "Input B", "Expected"], rows: [["1", "2", "3"], ["10", "20", "30"]] },
    docDeps,
  );
  const note = renderDocNote({ kind: "note", phase: "static", text: "This test verifies the calculator API." }, docDeps);
