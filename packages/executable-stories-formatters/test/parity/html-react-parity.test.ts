...
const reactCss = fs.readFileSync(
  createRequire(import.meta.url).resolve("executable-stories-react/tailwind.css"),
  "utf8",
);

const reactHtml = renderReportToHtml(report, { title: "Parity", css: reactCss });
...
function contentTokens(r: StoryReport): string[] {
  ...
  for (const feature of r.features) {
    for (const scenario of feature.scenarios) {
      ...
      for (const doc of scenario.docEntries ?? []) collectDocText(doc as Record<string, unknown>, tokens);
      for (const step of scenario.steps ?? []) {
        tokens.push(step.text);
        for (const doc of step.docEntries ?? []) collectDocText(doc as Record<string, unknown>, tokens);
      }
    }
  }
...
describe("html-react content gate", () => {
  ...
  it("React output surfaces every content token in the canonical report (react ⊇ report)", () => {
    ...
    const missingFromReact = tokens.filter((t) => !reactText.includes(stripWs(t)));

    expect(missingFromReact, `React dropped content present in the canonical report:\n${missingFromReact.join("\n")}`).toEqual([]);
  });

  it("renders the same scenario count as the canonical run", () => {
    const scenarioCount = report.features.reduce((n, f) => n + f.scenarios.length, 0);
