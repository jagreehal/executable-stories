...
function makeDeps(overrides: Partial<ShareDeps> = {}) {
  ...
  const deps: ShareDeps = {
    ...
    readBinary: vi
      .fn()
      .mockReturnValue(new Uint8Array([1, 2, 3]) as Uint8Array<ArrayBuffer>),
...
describe('resolveReport', () => {
  it('prefers the HTML report: it is the copy the Share button was clicked on', () => {
    ...
    const { deps } = makeDeps({
      listDir: vi
        .fn()
        .mockReturnValue(['index.html', 'index.story-report.json']),
  ...
  it('uses bundled paths from a custom-named HTML report', () => {
    ...
    bundled.features[0]!.scenarios[0]!.docEntries[0]!.path =
      'assets/basket-bundled.png';
    const { deps } = makeDeps({
      listDir: vi
        .fn()
        .mockReturnValue(['custom.html', 'custom.story-report.json']),
      readFile: vi.fn((file: string) =>
        file.endsWith('.html') ? htmlWith(bundled) : JSON.stringify(REPORT),
      ),
      fileSize: vi.fn((file: string) =>
        file.endsWith('basket-bundled.png') ? 10 : undefined,
      ),
    });
    const resolved = resolveReport('reports', deps);
  ...
  it('skips an HTML page with no report in it, like a colocated index', () => {
    const { deps } = makeDeps({
      ...
      readFile: vi.fn((file: string) =>
        file.endsWith('.html')
          ? '<html><body>a listing</body></html>'
          : JSON.stringify(REPORT),
      ),
    });
    expect(resolveReport('reports', deps).path).toBe(
  ...
  it('falls back to a raw run when there is no story report', () => {
    const { deps } = makeDeps({
      listDir: vi.fn().mockReturnValue(['raw-run.json']),
    });
    const rawRun = {
      schemaVersion: 1,
      runId: 'raw',
  ...
  it('says what to run when the directory holds no report', () => {
    const { deps } = makeDeps({
      listDir: vi.fn().mockReturnValue(['report.md']),
    });
...
describe('planAssets', () => {
  it('types each asset by extension so video plays and images render', () => {
    ...
    expect(
      assets.map(({ path, contentType, bytes }) => ({
        path,
        contentType,
        bytes,
      })),
...
describe('runShare', () => {
  it('creates the share, uploads every asset, then publishes it', async () => {
    ...
    expect(calls).toEqual([
      {
        url: 'https://app.executablestories.com/api/v1/shares',
        method: 'POST',
      },
      { url: 'https://store.example/put/1', method: 'PUT' },
      { url: 'https://store.example/put/2', method: 'PUT' },
      {
        url: 'https://app.executablestories.com/api/v1/shares/sh_abc/complete',
        method: 'POST',
      },
    ]);
  ...
  it('shares to a link by default and to named people when asked', async () => {
    ...
    await runShare(
      ['reports', '--emails', 'pm@acme.com, qa@acme.com'],
      second.deps,
    );
    const emailBody = JSON.parse(
      (second.fetchFn.mock.calls[0]![1] as RequestInit).body as string,
    );
  ...
  it('explains the free-tier limit in the words the user needs', async () => {
    const { deps } = makeDeps({
      fetchFn: vi
        ...
        .mockResolvedValue(
          new Response(
            JSON.stringify({ error: { type: 'SHARE_LIMIT', limit: 3 } }),
            { status: 409 },
          ),
  ...
  it('names what the plan did not store, and still publishes', async () => {
    const fetchFn = vi.fn(async (input: URL | string) => {
      const url = String(input);
      if (url.endsWith('/api/v1/shares')) {
        return new Response(
          JSON.stringify({
            ...
            notStored: [
              { path: 'assets/run.webm', reason: 'Free keeps screenshots' },
            ],
          }),
          { status: 201 },
        );
    ...
    expect(await runShare(['reports'], deps)).toBe(0);
    const out = vi.mocked(deps.log).mock.calls.flat().join('\n');
    expect(out).toContain(
      'Not stored: assets/run.webm (Free keeps screenshots)',
    );
    // Only the offered upload was attempted; the skipped file was never PUT.
    expect(
      fetchFn.mock.calls.filter(([u]) => String(u).includes('/put/')),
    ).toHaveLength(1);
  });

  it('explains the storage quota in the words the user needs', async () => {
    const { deps } = makeDeps({
      fetchFn: vi
        .fn()
        .mockResolvedValue(
  ...
  it('does not publish a share whose upload failed', async () => {
    ...
    expect(await runShare(['reports'], deps)).toBe(1);
    expect(
      fetchFn.mock.calls.some(([u]) => String(u).endsWith('/complete')),
    ).toBe(false);
  });
...
describe('asset keys never carry local layout', () => {
        ...
        scenarios: [
          {
            ...
            docEntries: [
              ...
              {
                kind: 'video',
                path: '/Users/someone/clients/acme/out/run.webm',
                phase: 'then',
              },
              { kind: 'screenshot', path: 'assets/basket.png', phase: 'then' },
            ],
  ...
  it('keys an outside-the-report asset by name, not by its path on this machine', () => {
    const { deps } = makeDeps();
    const { assets, keyByPath } = planAssets(
      ABSOLUTE_REPORT as never,
      'reports',
      deps,
    );
    ...
    expect(assets[0]?.localPath).toBe(
      '/Users/someone/clients/acme/out/run.webm',
    );
    expect(keyByPath.get('/Users/someone/clients/acme/out/run.webm')).toBe(
      'assets/run.webm',
    );
  });
      ...
      if (url.endsWith('/api/v1/shares')) {
        return new Response(
          JSON.stringify({
            ...
            url: 'https://app.executablestories.com/s/sh_abc',
            uploads: [
              { path: 'assets/run.webm', url: 'https://store.example/put/1' },
            ],
          }),
          { status: 201 },
        );
  ...
  it('keys a missing asset too, so its path on this machine is not shared', async () => {
    const { deps, fetchFn } = makeDeps({
      readFile: vi.fn().mockReturnValue(JSON.stringify(ABSOLUTE_REPORT)),
      fileSize: vi.fn((file: string) =>
        file.endsWith('run.webm') ? undefined : 10,
      ),
    });
    fetchFn.mockImplementation(async (input: URL | string) => {
      const url = String(input);
    ...
    expect(body).toContain('assets/run.webm');
    expect(
      JSON.parse(body).assets.map((a: { path: string }) => a.path),
    ).toEqual(['assets/basket.png']);
  });
  ...
  it('refuses to read a file the report never offered', async () => {
    ...
    expect(deps.error).toHaveBeenCalledWith(
      expect.stringContaining('did not offer'),
    );
  });
});
