...
import {
  toStoryReport,
  toStoryReportWithIndex,
} from 'executable-stories-core/converters/story-report';
import type {
  TestCaseResult,
  TestRunResult,
...
describe('toStoryReport', () => {
  ...
  it('groups test cases by sourceFile and pre-computes summaries at every level', () => {
    const run = makeRun([
      makeTestCase({
        id: '1',
        story: { scenario: 'Adds a todo', steps: [] },
      ...
      makeTestCase({
        id: '2',
        story: { scenario: 'Deletes a todo', steps: [] },
        sourceFile: '/repo/src/todos.test.ts',
        durationMs: 200,
        status: 'failed',
        errorMessage: 'boom',
      ...
      makeTestCase({
        id: '3',
        story: { scenario: 'Login works', steps: [] },
        sourceFile: '/repo/src/auth.test.ts',
        durationMs: 50,
        status: 'passed',
      }),
    ]);
    ...
    const todos = report.features.find(
      (f) => f.sourceFile === 'src/todos.test.ts',
    )!;
    expect(todos.sourceFile).toBe('src/todos.test.ts');
    expect(todos.scenarios).toHaveLength(2);
    expect(todos.summary).toEqual({
      total: 2,
      passed: 1,
      failed: 1,
      skipped: 0,
    ...
    const auth = report.features.find(
      (f) => f.sourceFile === 'src/auth.test.ts',
    )!;
    expect(auth.scenarios).toHaveLength(1);
    expect(auth.summary).toEqual({
      total: 1,
      passed: 1,
      failed: 0,
      skipped: 0,
  ...
  it('uses titlePath[0] as feature title when present, file basename otherwise', () => {
    const run = makeRun([
      makeTestCase({
        id: '1',
        titlePath: ['Todos suite', 'creates one'],
        sourceFile: '/repo/src/x.test.ts',
      }),
      makeTestCase({
        id: '2',
        titlePath: [],
        sourceFile: '/repo/src/y.test.ts',
      }),
    ]);
    const report = toStoryReport(run);
  ...
  it('preserves observed assertion counts in StoryReport steps', () => {
    const report = toStoryReport(
      makeRun([
        makeTestCase({
          story: {
            scenario: 'Checks its outcome',
            steps: [
              { keyword: 'Given', text: 'some setup', assertions: 0 },
              { keyword: 'Then', text: 'the outcome holds', assertions: 2 },
            ],
          },
          stepResults: [
            { index: 0, status: 'passed', durationMs: 1 },
            { index: 1, status: 'passed', durationMs: 1 },
          ],
        }),
      ]),
    );
    ...
    expect(
      report.features[0]!.scenarios[0]!.steps.map((step) => step.assertions),
    ).toEqual([0, 2]);
    expect(validateStoryReport(report).valid).toBe(true);
  });
  ...
  it('preserves tickets, retries, and tags', () => {
    const run = makeRun([
      makeTestCase({
        ...
        retries: 2,
        story: {
          scenario: 'with tickets',
          steps: [],
          tickets: [
            { id: 'JIRA-1', url: 'https://x/JIRA-1' },
            { id: 'JIRA-2' },
          ],
        },
      }),
  ...
  it('dedupes feature and scenario IDs that would collide', () => {
    const run = makeRun([
      makeTestCase({
        id: 'a',
        story: { scenario: 'Same Title', steps: [] },
        sourceFile: '/repo/src/foo.test.ts',
      }),
      makeTestCase({
        id: 'b',
...
describe('toStoryReportWithIndex', () => {
  it('maps every test case id to its final scenario id', () => {
    ...
    const allScenarioIds = report.features.flatMap((f) =>
      f.scenarios.map((s) => s.id),
    );

    expect(Object.keys(index.scenarioIdByTestCaseId).sort()).toEqual([
      'tc-a',
      'tc-b',
    ]);
  ...
  it('tracks the renamed id when duplicate scenario titles are de-duplicated', () => {
    ...
    expect(index.scenarioIdByTestCaseId['tc-1']).not.toBe(
      index.scenarioIdByTestCaseId['tc-2'],
    );
  });
...
describe('state docs in StoryReport', () => {
  it('round-trips state docs through StoryReportJsonFormatter and validates against the schema', () => {
    ...
    expect(steps[0].docEntries[0]).toMatchObject({
      kind: 'state',
      label: 'Basket',
      value: { items: [] },
    });
