...
namespace ExecutableStories.Xunit
{
    ...
    {
        private static readonly ConcurrentBag<RawTestCase> _testCases = [];
        ...
        {
            StoryContext? storyContext = Story.GetContext();
            List<Dictionary<string, object?>>? attachments = storyContext?.GetAttachments();
            ...
            durationMs = !string.IsNullOrEmpty(uniqueId) && _testStartTicks.TryRemove(uniqueId, out var startTicks)
                ? Stopwatch.GetElapsedTime(startTicks).TotalMilliseconds
                : (double)(executionTime * 1000m);
